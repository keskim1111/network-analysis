...
def run_cmd(command, run_path):
    logging.debug(f"Run path is {run_path}")
    logging.debug(f"current path = {os.getcwd()}")
    os.chdir(run_path)
    logging.debug(f"current cmd running = {command}")
...
def run_func_after_file_created(file_created_path, func, args):
    ...
    :return:
    '''
    logging.debug(" running run_func_after_file_created")
    while not os.path.exists(file_created_path):
        time.sleep(1)
    if os.path.isfile(file_created_path):
        logging.debug(f"file created in {file_created_path}")
        res = func(**args)
        return res
    else:
...
@timeit
def get_neumann_communities(save_dp, network_name, binary_input_fp, binary_output_fp=None, lp_critical=1):
    if binary_output_fp == None:
...
def split_communities_with_newman(save_dp, network_name, binary_graph_input_fp, communities):
    ...
    logging.debug(f"C_CODE: {C_CODE_SPLIT}")
